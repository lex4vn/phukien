...
jQuery(document).ready(function($){
	...
	jQuery('ul#top-nav').mobileMenu({
	  ...
	  jQuery('#bg_menu, .header-top .account, .header-top .cart').slideToggle('fast', function() {
	  	return false;
	    // Animation complete.
	  });
	});
	...
	if ( ! navigator.userAgent.match(/(iPod|iPhone|iPad|Android)/)) {
		...
		if (jQuery(window).width() > 768) {
			...
			if (jQuery(window).width() > 768) {
			   if (c > 30+pos.top){
				$('#bg_menu').css('position','fixed').css('top','0').css('width',$('.header-top').width()).addClass('fixed');
			   } else {
				$('#bg_menu').removeAttr('style').removeClass('fixed');
			   }
			} else {
			   $('#bg_menu').removeAttr('style').removeClass('fixed');
			}
			...
			$(window).resize(function(){
				if (jQuery(window).width() > 768) {
					...
					$('#bg_menu').css('width',$('#order_review .shop_table').width());
				}
		   	});
		}
